...
const masterCategories = {
  ...
  toBuy: {title: 'Products', tags:['shop', 'apparel', 'shopping', 'electronics', 'home', 'beauty']}
};

// const url = 'https://www.googleapis.com/customsearch/v1?fields=items(title,snippet)&num=10&key=AIzaSyDFcg70K1aUxF7w3yGLFvZb0sNy5EvucWQ&cx=012299370560497716930:1lnf0ccnkou&q=';// + phrase;
const getCategoryAPI = async(input) => {
  // async function getCategoryAPI(input) {
  const url = 'https://www.googleapis.com/customsearch/v1?fields=items(title,snippet)&num=10&key=AIzaSyDFcg70K1aUxF7w3yGLFvZb0sNy5EvucWQ&cx=012299370560497716930:1lnf0ccnkou&q=';
  ...
  try {
    ...
    Object.keys(masterCategories).map(key => {
      ...
      if(found) foundCats.push({key, title: masterCategories[key].title});
    });
    console.log('foundCats', foundCats);
    if (foundCats.length > 0) return foundCats;
    else if (result.categories[0]) return [{key:'misc', title: result.categories[0].name}];
    else return null;
  } catch (error) {
