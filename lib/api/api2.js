...
const masterCategories = {
  toWatch: { title: 'Film / Series', tags: ['film', 'serie', 'movie', 'online media', 'animation', 'videos'] },
      ...
      .then(promises => {
        Promise.all(promises)
          .then(res => {
            ...
            foundCats = checkToLocalCats(foundCats[0], true);
            return foundCats;
          })
          .then(res => {
            resolve(res);
...
const getCategoryAPI = async (input) => {
  ...
  if (bCat) return {key:'toBuy', title:'no-title'};
  let foundCats = [];
  try {
    ...
    foundCats = checkToLocalCats(result.categories[0]);
...
const checkToLocalCats = (result, misc = false) => {
  let foundCats = [];
  ...
  if (misc)
    foundCats =  !foundCats.length ? [{key:'misc', title: result.name}] : foundCats;
  return foundCats;
};
