...
const renderTasks = function(tasks) {
  for (let task of tasks) {
    ...
    if (task.status_id === 2) {
      ...
      $('#' + task.key).append(`
        <li class="list-group-item" id="task-${task.id}" class="draggable" draggable="true" ondragstart="drag(event)" data-toggle="modal" data-target="#taskModal" data-name="${task.title}" data-desc="${task.description}" data-created='${task.date_created}'>
